...
/*
		...
		std::cout<<"GA begin!\n";
		mOppGeneme = mTeam->getOpponent()->getAssignedRegionIDs();
		unsigned int tmp_size = mOppGeneme.size();
		...
		tmp_size = mPopulatin.getBestGeneme().size();
		std::cout << "\n-----------------size: " << tmp_size << "\n";
		for(unsigned int i = 0; i < tmp_size; ++i)
			std::cout << "\nbestgene: " << mPopulatin.getBestGeneme()[i] << "\n";
		std::cout<<"--------------------------\n";

		unsigned int player_size = mTeam->getPlayers().size();
		for(unsigned int i = 0; i < player_size; ++i)
		{
